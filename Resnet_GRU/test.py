...
def test(model, data):  #c, h, w
    ...
    with torch.no_grad():
        ...
        #data = data.reshape((Batch, C, H, W))
        data = torch.from_numpy(data).float()
        data = data.to('cpu')

        output = model(data)
        
        output = output.sum(1) #b,class ==> 1,class
        pred = output.data.cpu().max(1)[1] 

        #print('prediction time : {}, predict : {}'.format(time.time()-start_time, pred))
...
if __name__ == "__main__":
    ...
    origin = ResLSTMNet(BasicBlock1, [1, 2, 5, 3])
    origin = origin.to('cpu')

    checkpoint_dir = 'D:/data/face_data/checkpoint/2020-05-05_models_epoch_0451_loss_0.008156_acc_0.943.pth'
    checkpoint = torch.load(checkpoint_dir)
    model.load_state_dict(checkpoint['model'])
    images = np.random.rand(4,1,64,64)
    print(type(images))
