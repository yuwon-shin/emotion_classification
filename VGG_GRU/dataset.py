...
class listDataset(Dataset):
    ...
    def __init__(self, root, train=False, dataset = None ):
        ...
        # self.nSamples = len(next(os.walk(root))[2]) #len(self.lines[:60]) if debug  else len(self.lines)
    ...
    def __getitem__(self, root):
        ...
        for (path, dir, files) in os.walk(root):
            for filename in files:
                imgpath = os.path.splitext(filename)[-1]
    ...
    def load_data_label(self,imgpath):
        ...
        seq = np.zeros((224, 224, 3), dtype=np.float32)
        ...
        img_file = imgpath
        label_index = imgpath.split("_")[-1]
        # label_index = int(classes.index(label_path))

        # file_length = len(os.listdir(img_path))
        ...
        #     else:
        #         img_file = os.path.join(img_path,imgs[file_length-1])

        img =  Image.open(img_file)     #.convert('RGB')
        img = data_augmentation(img,(224,224), jitter, hue, saturation, exposure)
        img = np.array(img)

        seq[:, :, 0] = (img[:,:,0]/255.-mean[0])/std[0]
        seq[:, :, 1] = (img[:,:,1]/255.-mean[1])/std[1]
        seq[:, :, 2] = (img[:,:,2]/255.-mean[2])/std[2]
    ...
    #     classes = []
    #     for line in open(self.dataset+'/classInd.txt'):
